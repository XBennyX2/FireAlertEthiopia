...
<div className="section-label">{t.myReports}</div>
        ...
        {!loading && !error && incidents.length === 0 && (
          <div className="empty-state">
            ...
            <div>{t.noReports}</div>
            <Link to="/report" className="btn-primary" style={{ marginTop:'1rem', display:'inline-block' }}>
              {t.submitFirst}
            </Link>
          </div>
        )}
        ...
        {!loading && incidents.length > 0 && (
          <div className="incident-grid">
            {incidents.map(incident => {
              ...
              return (
                <div key={incident._id} className="card" style={{ display:'flex', flexDirection:'column', gap:'0.75rem' }}>
                  ...
                  {/* Footer */}
                  <div style={{ borderTop:'1px solid #1e1e1e', paddingTop:'0.65rem', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                    <span style={{ fontSize:'0.72rem', color:'#555' }}>
                      📍 {incident.location?.address || `${incident.location?.lat?.toFixed(4)}, ${incident.location?.lng?.toFixed(4)}`}
                    </span>
                    <span style={{ fontSize:'0.72rem', color:'#444' }}>{fmtDate(incident.reportedAt)}</span>
                  </div>
