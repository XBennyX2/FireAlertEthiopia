...
async function sendEmail({ to, subject, html }) {
  await resend.emails.send({
    from: process.env.EMAIL_FROM || 'FireAlert <onboarding@resend.dev>',
    to,
    subject,
    html,
  });
  ...
  const html = `
    <div style="font-family:Arial,sans-serif;max-width:480px;margin:0 auto;background:#0e0e0e;color:#f0ede8;padding:2rem;border-radius:12px;">
      <div style="text-align:center;margin-bottom:2rem;">
        ...
        <h1 style="font-size:1.25rem;font-weight:800;margin:0.75rem 0 0;color:#f0ede8;">FireAlert</h1>
      </div>
...
async function sendPasswordResetEmail(toEmail, resetLink) {
  const html = `
    <div style="font-family:Arial,sans-serif;max-width:480px;margin:0 auto;background:#0e0e0e;color:#f0ede8;padding:2rem;border-radius:12px;">
      ...
      <h2 style="font-size:1rem;font-weight:600;color:#f0ede8;margin:0 0 0.5rem;">Reset Your Password</h2>
      <p style="font-size:0.875rem;color:#888;line-height:1.6;margin:0 0 1.5rem;">
        You requested a password reset. Click the button below to set a new password.
        This link expires in <strong style="color:#f4820a;">1 hour</strong>.
      </p>
      <div style="text-align:center;margin-bottom:1.5rem;">
        <a href="${resetLink}" style="display:inline-block;padding:0.875rem 2rem;background:linear-gradient(135deg,#e63c2f,#f4820a);color:#fff;text-decoration:none;border-radius:8px;font-weight:700;font-size:0.9375rem;">
          Reset Password
        </a>
      </div>
      <div style="background:#161616;border:1px solid #2a2a2a;border-radius:8px;padding:0.75rem 1rem;margin-bottom:1.5rem;">
        <div style="font-size:0.72rem;color:#555;margin-bottom:0.3rem;">Or copy this link:</div>
        <div style="font-size:0.72rem;color:#888;word-break:break-all;">${resetLink}</div>
      </div>
      <p style="font-size:0.78rem;color:#555;margin:0;">
  ...
  await sendEmail({ to: toEmail, subject: 'FireAlert — Reset Your Password', html });
}

// ── Send status update email ──────────────────────────────────────
async function sendStatusUpdateEmail(toEmail, userName, status, fireType) {
  const messages = {
    ...
    rejected:   { subject:'Update on your fire report',             body:'After assessment, your report could not be verified.' },
  };
  ...
  const html = `
    <div style="font-family:Arial,sans-serif;max-width:480px;margin:0 auto;background:#0e0e0e;color:#f0ede8;padding:2rem;border-radius:12px;">
      ...
      <div style="background:#161616;border:1px solid #2a2a2a;border-radius:8px;padding:1rem;margin-bottom:1rem;">
        ...
        <div style="font-size:.9rem;font-weight:600;text-transform:capitalize;">${fireType}</div>
      </div>
      <p style="font-size:.75rem;color:#555;">Open the FireAlert app to view full details.</p>
    </div>
  `;

  await sendEmail({ to: toEmail, subject: info.subject, html });
}
